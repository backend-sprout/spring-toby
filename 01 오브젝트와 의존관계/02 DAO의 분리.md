...
그리고 getConncetion() 메서드와 같이, 추상 메서드를 정의하고  
서브 클래스에서 구체적인 오브젝트를 생성하는 방법을 결정하는 것을 **팩토리 메서드 패턴**이라고 한다.  

좀 헷갈릴만한데, getConnection()이 void 이거나     
별도의 구현체 타입이 아닌 String, Int 또는 단순 클래스일 경우는 **팩토리 메서드 패턴**이 아니라고 보면 된다.  
   
`getConncetion()`으로 반환된 구현체가 무엇인지 신경쓰지 않아도 된다.      
Dao는 그저 Connection 이 제공해주는 기능에 신경을 쓸 뿐이고 이는 기능 제공에 관심사를 두고 있다는 뜻이다.        
이렇게 내부 방식에는 상관없이 자신이 필요한 기능을 서브 클래스에서 구현하여 사용하는 방식을 우리는 몸에 익혀야한다.    

![image (1)](https://user-images.githubusercontent.com/50267433/161973099-8cf35c77-6a69-46b6-812b-edff0960f4a9.png)
 
상위 인터페이스를 기준으로 생성된 구현체들을 반환하도록 하는 팩토리 메서드 패턴을 나타내는 그림이다.       
중요한건 상속/구현과정을 통해 성격이 다른 관심사항을 분리한 코드를 만들어내고, 서로 영향이 덜 주도록 했는지를 이해하는 것이다.   
   
이렇게 템플릿 메서드 패턴, 팩토리 메서드 패턴으로 관심사항이 다른 코드를 분리해냈고       
서로 독립적으로 변경 또는 확장 할 수 있도록 만드는 것은 간단하면서도 매우 효과적인 방법이다.         
    
하지만, 상속을 사용하는 위 예시는 몇가지 단점들이 있다.      
자바는 다중 상속을 허용하지 않으며, 상속을 위한 상위 오브젝트 생성을 위한 생성자 구문도 추가해야한다.     
무엇 보다도 상위 클래스가 바뀌면 하위 클래스가 영향을 받게되는 **상속에 강하게 결합되고 의존된 모습이다.**      
슈퍼클래스가 변하지 않도록 제약을 줄수도 있지만, 이보다는 다른 방법을 강구하는 것이 나을 것이다.     
